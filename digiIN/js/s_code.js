...
function s_doPlugins(s) {
    ...
    //if (s.pageName == "http://www.dbsmarinaregatta.com") s.pageName="Regatta:Home";
	//if (s.pageName == "http://www.posb.com.sg") s.pageName="http://www.dbs.com/posb";
	//if (s.pageName == "sg") s.prop1="sg";
	
	/* Change pagename for pages that sit outside of sg/personal */
	var directoryList=new Array("contact","posb","ratesonline","tools","treasures")
	for (i=0; i<directoryList.length; ++i) {
		if(s.pageName.substr(0,directoryList[i].length)==directoryList[i]){s.pageName="sg:personal:"+s.pageName;}
	}
      ...
      /* Page Name Clean Up Code */
		...
		/* sharepoint */
        s.pageName=s.pageName.replace(/home:default.aspx/,"home:index.html");
        s.pageName=s.pageName.replace(/:default.aspx/,"");
        s.pageName=s.pageName.replace(new RegExp(/.aspx/g),".html");
			
		//to replicate ".aspx" treatment to ".page"
		s.pageName=s.pageName.replace(/home:default.page/,"home:index.html");
        s.pageName=s.pageName.replace(/:default.page/,"");
        s.pageName=s.pageName.replace(new RegExp(/.page/g),".html");
			
		//remove all occurence of "?" to prevent duplicate pagename entry
		s.pageName = s.pageName.replace(/\?/g, "");
		
		//check for ending of "dbs.com.sg" for singapore domain
		curr_domain = window.location.hostname;
		curr_TLD = curr_domain.substring(curr_domain.length-10); 
		if (curr_TLD == "dbs.com.sg") {
			if (s.pageName == "") {
				s.pageName = "sg";
			} else {
				var pagenamearray = s.pageName.split(":");
				if (pagenamearray[0]!="sg") {
					s.pageName = "sg:" + s.pageName;
				}
			}
		}
		
		//check for ending of "posb.com.sg" for posb domain, replace "personal:" with "sg:personal:posb:"
		curr_domain = window.location.hostname;
		curr_TLD = curr_domain.substring(curr_domain.length-11); 
		if (curr_TLD == "posb.com.sg") s.pageName=s.pageName.replace("personal","sg:personal:posb");
		
		//special handling for sg landing page
		//curr_url = window.location.href;
		//if (curr_url=="http://www.dbs.com.sg/") s.pageName = "sg";
		
			
	/* Break up Page Name into sections and sub sections  */
		var valueArray = s.pageName.split(":");
		if(valueArray[1]!=undefined) {
			s.prop1=valueArray[0];		//Site
			s.hier1=valueArray[0];	
			if(valueArray[2]!=undefined) {
				s.channel=valueArray[0]+':'+valueArray[1];		//Section
				s.hier1=valueArray[0]+'|'+valueArray[1]; 	
				if(valueArray[3]!=undefined) {
					s.prop2=valueArray[0]+':'+valueArray[1]+':'+valueArray[2];	//Sub Section
					s.hier1=valueArray[0]+'|'+valueArray[1]+'|'+valueArray[2];
					if(valueArray[4]!=undefined) {
						s.prop3=valueArray[0]+':'+valueArray[1]+':'+valueArray[2]+':'+valueArray[3];	//Sub Sub Section
						s.hier1=valueArray[0]+'|'+valueArray[1]+'|'+valueArray[2]+'|'+valueArray[3];
						if(valueArray[5]!=undefined) {
							s.prop4=valueArray[0]+':'+valueArray[1]+':'+valueArray[2]+':'+valueArray[3]+':'+valueArray[4];	//Sub Sub Sub Section
  ...
  /* manageQueryParam v1.2 */
	...
	/* Channel Manager */
		s.channelManager('cid,gid,s_cid,sc_cid',':','s_cm','');
  ...
  /* Newsroom tracking */
  if(s.prop1 == "newsroom") {
    s.eVar17=s.getQueryParam('startdate,enddate','~');
    s.eVar18=s.getQueryParam('newsid');
    s.eVar19=s.getQueryParam('country');
    s.eVar20=s.getQueryParam('location');
  }
}
...
/* Top 130 */
s.seList="paran.com|q,r|Paran>aol.co.uk,search.aol.co.uk|query"
